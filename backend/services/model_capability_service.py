...
class ModelCapabilityService:
    """Persist measured model limits independently from assignments."""
    ...
    def get_profile(
        ...
        if profile is None:
            raise ValueError(
                f"No capability profile exists for "
                f"{provider_id}/{model}"
            )
    ...
    def save_profile(
        ...
        if profile["safe_input_tokens"] is not None:
            ...
            if profile["safe_input_tokens"] > maximum_safe_input:
                raise ValueError(
                    "safe_input_tokens must leave room for "
                    "max_output_tokens"
                )
    ...
    def record_benchmark(
        ...
        *,
        ...
        """Merge one measured stage result without changing assignments."""

        if stage not in {"planning", "generation", "repair"}:
            raise ValueError(
                f"Unsupported benchmark stage: {stage}"
            )
        if score < 0.0 or score > 1.0:
            raise ValueError(
                "Benchmark score must be between 0 and 1"
            )
    ...
    def recommend_assignments(self) -> Dict[str, Any]:
        """Rank benchmarked models per stage; never apply them."""

        profiles = [
            profile
            for profile in self.list_profiles()
            if profile.get("benchmarked_at")
            and profile.get("structured_output_mode")
            != "unsupported"
        ]
    ...
    def resolve_runtime(
        ...
        """Clamp user generation settings to model capabilities."""

        provider_id = str(runtime["provider_id"])
        model = str(runtime["model"])
        generation = runtime.get("generation", {})
        configured_context = int(
            generation.get("context_window", 8192)
        )
        configured_output = int(
            generation.get("max_tokens", 2048)
        )
        key = self._key(provider_id, model)
        saved = self._read()["profiles"].get(key)

        if saved is None:
            provider_kind = runtime.get(
                "provider",
                {},
            ).get("kind")
            structured_mode = (
        ...
        if safe_input is None:
            safe_input = max(
                512,
                effective_context - effective_output - 768,
            )
        else:
            safe_input = min(
                int(safe_input),
                max(
                    512,
                    effective_context - effective_output,
                ),
            )
    ...
    def require_structured_stage(
        ...
        if profile["structured_output_mode"] == "unsupported":
            raise ValueError(
                f"Model '{profile['model']}' is marked as unable "
                f"to produce structured output required by the "
                f"{stage} stage"
            )
        return profile
    ...
    def _read(self) -> Dict[str, Any]:
        with self._lock:
            try:
                ...
                raise RuntimeError(
                    f"Could not read model capability settings: "
                    f"{error}"
                ) from error

            if not isinstance(document, dict):
                raise RuntimeError(
                    "Model capability settings must be an object"
                )
            document.setdefault("version", 1)
            document.setdefault("profiles", {})
            if not isinstance(document["profiles"], dict):
                raise RuntimeError(
                    "Model capability profiles must be an object"
                )
            return document
    ...
    def _key(provider_id: str, model: str) -> str:
        ...
        if not clean_provider or not clean_model:
            raise ValueError(
                "provider_id and model must be non-empty"
            )
