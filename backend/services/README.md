...
- persistent change proposals and verification-linked repair tasks
...
CHANGE_DB_PATH=data/changes.sqlite3
REPAIR_DB_PATH=data/repairs.sqlite3
```
...
Failed verification runs can be converted into persistent repair tasks. Agent
proposals made for that task carry both a repair-task ID and a per-turn change
set ID. After human approval, run the task's original verification profile
again; the result closes the task only when the check passes.

See [Safe Repair Tasks](../docs/safe-repair-tasks.md) for the lifecycle and API.
