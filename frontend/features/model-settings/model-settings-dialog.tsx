    ...
    return (
        ...
        >
            ...
            <DialogContent className="max-w-4xl">
                ...
                {loading ? (
                    <div className="flex h-60 items-center justify-center text-sm text-muted-foreground"><Loader2Icon className="mr-2 size-5 animate-spin" />Loading settings…</div>
                ) : snapshot ? (
                        ...
                        <div className="grid gap-6 py-3 lg:grid-cols-[280px_1fr]">
                            <section className="space-y-3">
                                ...
                                {snapshot.providers.map((provider) => (
                                    <div key={provider.id} className="rounded-lg border border-border p-3 dark:border-border">
                                        <div className="flex items-start justify-between gap-2">
                                            <div><p className="text-sm font-medium">{provider.name}</p><p className="text-xs text-muted-foreground">{provider.kind.replace("_", " ")}</p></div>
                                        ...
                                        <p className="mt-2 break-all text-[11px] text-muted-foreground">{provider.base_url}</p>
                                ...
                                {newProvider ? (
                                    <div className="space-y-3 rounded-lg border border-pending/30 p-3 dark:border-pending/30">
                            ...
                            <section className="space-y-4">
                                ...
                                {agentSettings ? <>
                                    ...
                                    <div><Label>Model</Label><Select value={agentSettings.model} disabled={busy || models.length === 0} onValueChange={(model) => void updateAgent({ model })}><SelectTrigger><SelectValue placeholder={models.length ? "Select an installed model" : "No discovered models"} /></SelectTrigger><SelectContent portalContainer={portalContainer}>{models.map((model) => <SelectItem key={model.name} value={model.name}>{model.name} · {readableBytes(model.size)}</SelectItem>)}</SelectContent></Select><p className="mt-1 text-xs text-muted-foreground">{models.length ? `${models.length} installed model(s) found. Selection saves immediately.` : "Test the provider or confirm Ollama is running."}</p></div>
                                    {selectedModel?.warnings.map((warning) => <div key={warning} className="flex gap-2 rounded-lg border border-pending/30 bg-pending/10 p-3 text-xs text-pending dark:border-pending/30 dark:bg-pending/10 dark:text-pending"><AlertTriangleIcon className="size-4 shrink-0" />{warning}</div>)}
                                    {selectedModel ? <p className="text-xs text-muted-foreground">Installed size: {readableBytes(selectedModel.size)}</p> : null}
