...
function statusClass(status: RepairTask["status"]) {
  return cn(
    ...
    status === "passed" && "border-success/30 bg-success/10 text-success",
    status === "failed" && "border-danger/30 bg-danger/10 text-danger",
    status === "awaiting_review" && "border-pending/30 bg-pending/10 text-pending",
    status === "ready_to_verify" && "border-pending/30 bg-pending/10 text-pending",
    status === "open" && "border-border bg-surface-raised text-muted-foreground",
  );
}
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto">
        ...
        <div className="space-y-3">
          {tasks.map((task) => (
            <article key={task.task_id} className="rounded-xl border border-border bg-surface-raised p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    {task.status === "passed" ? <CheckCircle2Icon className="size-4 text-success" /> : <CircleDotIcon className="size-4 text-pending" />}
                  ...
                  <code className="mt-2 block truncate text-xs text-muted-foreground" title={task.display_command}>{task.display_command}</code>
          ...
          {!loading && tasks.length === 0 ? (
            <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">No active repair tasks for this workspace.</p>
          ) : null}
        </div>
      </DialogContent>
    </Dialog>
